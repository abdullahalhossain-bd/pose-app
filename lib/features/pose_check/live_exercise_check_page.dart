...
class _LiveExerciseCheckPageState extends State<LiveExerciseCheckPage> with WidgetsBindingObserver {
  ...
  List<PosePoint> _points(Iterable<dynamic> landmarks) => landmarks.map<PosePoint>((x) {
        final rawName = x.type.toString().split('.').last;
        return PosePoint(
          name: rawName,
  ...
  Future<void> _onFrame(CameraImage image) async {
    ...
    try {
      ...
      if (!result.hasPoses) {
        if (mounted) setState(() => status = 'FIND YOUR BODY');
        return;
      }
      ...
      if (out.validRep) {
        reps++;
        repScores.add(out.score.clamp(0, 100));
      }
      if (mounted) setState(() {
        pose = p;
        feedback = out;
        status = out.validRep ? 'REP COMPLETE' : out.score >= 75 ? 'GOOD' : 'MOVE';
      });
    } catch (_) {
  ...
  Future<void> _stop({bool summary = true}) async {
    ...
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => SessionCompletePage(
      exercise: widget.exerciseId, reps: scores.length, score: avg, bestScore: best, repScores: scores,
    )));
  }

  @override void didChangeAppLifecycleState(AppLifecycleState s) {
    if (s != AppLifecycleState.resumed && running) _stop(summary: false);
  }
    ...
    return Scaffold(
      ...
      body: Stack(fit: StackFit.expand, children: [
        ...
        SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(16,10,16,0), child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('${exercise.name} Check',style:const TextStyle(color:Colors.white,fontSize:19,fontWeight:FontWeight.w900)),const Text('Live form coaching',style:TextStyle(color:Colors.white60,fontSize:12))])),
          IconButton(onPressed: running ? null : () async { front = !front; await _initCamera(); }, icon: const Icon(Icons.flip_camera_android_rounded,color:Colors.white)),
        ]))),
        ...
        if (!running) Positioned(top:150,left:28,right:28,child:Container(padding:const EdgeInsets.all(15),decoration:BoxDecoration(color:Colors.black54,borderRadius:BorderRadius.circular(18)),child:Text('${exercise.description}\n\n${analyzer?.setupTips.join(' • ') ?? ''}',style:const TextStyle(color:Colors.white,fontSize:12,height:1.4,fontWeight:FontWeight.w600)))),
        Positioned(left:18,right:18,bottom:18,child:Container(padding:const EdgeInsets.all(18),decoration:BoxDecoration(color:const Color(0xEE171720),borderRadius:BorderRadius.circular(25)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          Row(children:[_Stat('REPS','$reps'),const SizedBox(width:10),_Stat('SCORE',f == null ? '—' : '${f.score}'),const SizedBox(width:10),_Stat('PHASE',f == null ? 'Ready' : f.phase.name)]),
          const SizedBox(height:14),Text(f?.messages.isNotEmpty == true ? f!.messages.first : (running ? 'Follow the movement cues.' : 'Ready when you are.'),style:const TextStyle(color:Colors.white,fontSize:14,fontWeight:FontWeight.w700)),
          const SizedBox(height:14),SizedBox(width:double.infinity,height:52,child:FilledButton(onPressed:c == null ? null : (running ? _stop : _start),style:FilledButton.styleFrom(backgroundColor:const Color(0xFF6C5CE7),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(16))),child:Text(running ? 'Finish session' : 'Start ${exercise.name}',style:const TextStyle(fontWeight:FontWeight.w900))))
        ])),
      ]),
    );
  }
