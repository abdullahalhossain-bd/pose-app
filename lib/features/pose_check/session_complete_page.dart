...
class _SessionCompletePageState extends State<SessionCompletePage> {
  ...
  Future<void> _trainAgain() async {
    await Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => widget.exercise == 'squat' ? const LiveSquatCheckPage() : LiveExerciseCheckPage(exerciseId: widget.exercise)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 12, 20, 30), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [IconButton(tooltip: 'Close results', onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded)), const Expanded(child: Text('SESSION COMPLETE', textAlign: TextAlign.center, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.5, color: _muted))), const SizedBox(width: 48)]),
        const SizedBox(height: 10),
        Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(22, 24, 22, 22), decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [_purpleDark, Color(0xFF8B79FF)]), borderRadius: BorderRadius.circular(30), boxShadow: const [BoxShadow(color: Color(0x337B68EE), blurRadius: 28, offset: Offset(0, 12))]), child: Column(children: [
          Container(width: 54, height: 54, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(18)), child: Icon(item.icon, color: Colors.white, size: 28)),
          const SizedBox(height: 13),
          Text(item.name.toUpperCase(), style: const TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
          const SizedBox(height: 5),
          const Text('Your form score', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w800)),
          const SizedBox(height: 4),
          Text('${widget.score}', style: const TextStyle(color: Colors.white, fontSize: 64, height: .98, fontWeight: FontWeight.w900)),
          const Text('/100', style: TextStyle(color: Colors.white60, fontSize: 13, fontWeight: FontWeight.w700)),
          const SizedBox(height: 12),
          Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 7), decoration: BoxDecoration(color: Colors.white18, borderRadius: BorderRadius.circular(30)), child: Text(label, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12))),
        ])),
        const SizedBox(height: 22),
        const Text('SESSION SNAPSHOT', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.4, color: _muted)),
        const SizedBox(height: 10),
        Row(children: [Expanded(child: _StatCard(Icons.repeat_rounded, '${widget.reps}', 'Reps')), const SizedBox(width: 10), Expanded(child: _StatCard(Icons.emoji_events_rounded, has ? '${widget.bestScore}' : '—', 'Best')), const SizedBox(width: 10), Expanded(child: _StatCard(Icons.trending_up_rounded, has ? '${widget.score}' : '—', 'Average'))]),
        const SizedBox(height: 22),
        ...
        if (_saved) const Center(child: Padding(padding: EdgeInsets.only(bottom: 8), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.check_circle_rounded, size: 15, color: _purple), SizedBox(width: 5), Text('Saved on this device', style: TextStyle(color: _muted, fontSize: 11, fontWeight: FontWeight.w700))]))),
        SizedBox(width: double.infinity, height: 54, child: FilledButton.icon(onPressed: _trainAgain, icon: const Icon(Icons.replay_rounded), label: Text('Train ${item.name.toLowerCase()} again', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15)), style: FilledButton.styleFrom(backgroundColor: _purple, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(17))))),
        const SizedBox(height: 10),
        SizedBox(width: double.infinity, height: 50, child: OutlinedButton.icon(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.arrow_back_rounded, size: 19), label: const Text('Back to training', style: TextStyle(fontWeight: FontWeight.w800)), style: OutlinedButton.styleFrom(foregroundColor: _ink, side: const BorderSide(color: Color(0xFFDCD8E7)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))))),
        const SizedBox(height: 14),
        const Center(child: Text('Results are processed and stored locally on your device.', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFF9995A3), fontSize: 10.5))),
      ])),
    );
  }
}
