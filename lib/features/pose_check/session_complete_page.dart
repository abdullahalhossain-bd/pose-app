...
class _SessionCompletePageState extends State<SessionCompletePage> {
  static final LocalSessionStore _store = LocalSessionStore.instance;
  bool _saved = false;
  ...
  Future<void> _saveSession() async {
    ...
    await _store.add(WorkoutSession(exercise: 'squat', reps: widget.reps, averageScore: widget.score.clamp(0, 100), bestScore: widget.bestScore.clamp(0, 100), repScores: scores, completedAt: widget.completedAt ?? DateTime.now(), durationSeconds: widget.durationSeconds));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 16, 20, 28), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 22), const Text('YOUR SESSION', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.3, color: _muted)), const SizedBox(height: 10),
        ...
        if (_saved) const Center(child: Text('Saved on this device', style: TextStyle(color: _muted, fontSize: 11, fontWeight: FontWeight.w600))),
        ...
        SizedBox(width: double.infinity, height: 50, child: OutlinedButton.icon(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.insights_rounded, size: 20), label: const Text('View progress', style: TextStyle(fontWeight: FontWeight.w800)), style: OutlinedButton.styleFrom(foregroundColor: _ink, side: const BorderSide(color: Color(0xFFDCD8E7)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))))),
        const SizedBox(height: 12), const Center(child: Text('Results are processed and stored locally on your device.', style: TextStyle(color: Color(0xFF9995A3), fontSize: 11))),
      ])),
    );
  }
}
