...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> openCheck() async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => LiveSquatCheckPage(sessionStore: store),
      ),
    );
    await store.load();
    if (mounted) setState(() {});
  ...
  Widget build(BuildContext context) {
    final pages = [
      _Dashboard(onStart: openCheck, sessions: store.sessions),
      _WorkoutsPage(onStart: openCheck),
      _ProgressPage(sessions: store.sessions),
      const _ProfilePage(),
    ];
    return Scaffold(
...
class _Dashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final todaySessions = sessions.where((s) => s.completedAt.year == today.year && s.completedAt.month == today.month && s.completedAt.day == today.day).toList();
    final todayReps = todaySessions.fold<int>(0, (sum, s) => sum + s.reps);
    final todayMinutes = todaySessions.fold<int>(0, (sum, s) => sum + (s.durationSeconds / 60).ceil());
    ...
    return ListView(
      ...
      children: [
        ...
        Row(children: [
          Expanded(child: _Metric(value: '${todaySessions.length}', label: 'Sessions', icon: Icons.local_fire_department_outlined)),
          const SizedBox(width: 10),
          Expanded(child: _Metric(value: '$todayMinutes', label: 'Minutes', icon: Icons.schedule_rounded)),
          const SizedBox(width: 10),
          Expanded(child: _Metric(value: best == null ? '—' : '$best', label: 'Best score', icon: Icons.auto_awesome_outlined)),
        ]),
...
class _IconButton extends StatelessWidget {
  ...
  final IconData icon;
  @override
  Widget build(BuildContext context) => Container(width: 46, height: 46, decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFEDEAF2))), child: Icon(icon, color: _ink));
}
...
class _ProgressPage extends StatelessWidget {
  const _ProgressPage({required this.sessions});
  final List<WorkoutSession> sessions;
  @override
  Widget build(BuildContext context) {
    final reps = sessions.fold<int>(0, (sum, s) => sum + s.reps);
    final best = sessions.isEmpty ? null : sessions.map((s) => s.bestScore).reduce((a, b) => a > b ? a : b);
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 28), children: [
      ...
      const SizedBox(height: 6), const Text('Small improvements add up.', style: TextStyle(color: _muted, fontSize: 14)), const SizedBox(height: 24),
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(26), border: Border.all(color: const Color(0xFFEDEAF2))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Expanded(child: Text('Your journey', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: _ink))), Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: const Color(0xFFEFEAFF), borderRadius: BorderRadius.circular(20)), child: Text(hasData ? 'ACTIVE' : 'STARTING', style: const TextStyle(color: _purple, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)))]),
        const SizedBox(height: 20), Row(children: [Expanded(child: _ProgressStat('${sessions.length}', 'Sessions')), Expanded(child: _ProgressStat('$reps', 'Reps')), Expanded(child: _ProgressStat(best == null ? '—' : '$best', 'Best score'))]),
        const SizedBox(height: 18), Text(hasData ? 'Average score  $avg / 100' : 'Complete your first session to start tracking.', style: const TextStyle(color: _muted, fontSize: 13, fontWeight: FontWeight.w600)),
      ])),
      const SizedBox(height: 18),
      if (!hasData) const _EmptyProgressCard(),
      if (hasData) ...[
...
class _SessionHistoryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFEDEAF2))), child: Row(children: [
      Container(width: 46, height: 46, decoration: BoxDecoration(color: const Color(0xFFEFEAFF), borderRadius: BorderRadius.circular(14)), child: const Icon(Icons.directions_run_rounded, color: _purple)),
      const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Squat', style: TextStyle(fontWeight: FontWeight.w900, color: _ink)), const SizedBox(height: 4), Text('${session.reps} reps • $date', style: TextStyle(color: _muted, fontSize: 12))])),
      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('${session.averageScore}', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: _ink)), const Text('score', style: TextStyle(fontSize: 10, color: _muted))]),
    ]));
  }
}

class _ProgressStat extends StatelessWidget { const _ProgressStat(this.value, this.label); final String value, label; @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.w900, color: _ink)), const SizedBox(height: 3), Text(label, style: const TextStyle(color: _muted, fontSize: 12))]); }
