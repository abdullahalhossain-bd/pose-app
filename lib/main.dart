...
class PoseApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        theme: ThemeData(useMaterial3: true, scaffoldBackgroundColor: _bg, colorScheme: ColorScheme.fromSeed(seedColor: _purple), fontFamily: 'sans'),
        home: const HomePage(),
      );
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> openCheck() async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => const LiveSquatCheckPage()));
    await store.load();
    if (mounted) setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        height: 74, backgroundColor: _surface, elevation: 0, indicatorColor: const Color(0xFFEAE6FF), selectedIndex: index,
...
class _ProgressPage extends StatelessWidget {
  const _ProgressPage({required this.sessions}); final List<WorkoutSession> sessions;
  @override Widget build(BuildContext context) {
    ...
    final recent = sessions.take(5).toList(); final hasData = sessions.isNotEmpty;
    return ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 28), children: [const Text('Progress', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, color: _ink)), const SizedBox(height: 6), const Text('Small improvements add up.', style: TextStyle(color: _muted, fontSize: 14)), const SizedBox(height: 24),
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(26), border: Border.all(color: const Color(0xFFEDEAF2))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Expanded(child: Text('Your journey', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: _ink))), Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: const Color(0xFFEFEAFF), borderRadius: BorderRadius.circular(20)), child: Text(hasData ? 'ACTIVE' : 'STARTING', style: const TextStyle(color: _purple, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)))]), const SizedBox(height: 20), Row(children: [Expanded(child: _ProgressStat('${sessions.length}', 'Sessions')), Expanded(child: _ProgressStat('$reps', 'Reps')), Expanded(child: _ProgressStat(best == null ? '—' : '$best', 'Best score'))]), const SizedBox(height: 18), Text(hasData ? 'Average score  $avg / 100' : 'Complete your first session to start tracking.', style: const TextStyle(color: _muted, fontSize: 13, fontWeight: FontWeight.w600))])),
      const SizedBox(height: 18), if (!hasData) const _EmptyProgressCard(), if (hasData) ...[const Text('Recent sessions', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: _ink)), const SizedBox(height: 12), ...recent.map((s) => _SessionHistoryCard(session: s))]
    ]);
  }
}
class _SessionHistoryCard extends StatelessWidget { const _SessionHistoryCard({required this.session}); final WorkoutSession session; @override Widget build(BuildContext context) => Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFEDEAF2))), child: Row(children: [Container(width: 46, height: 46, decoration: BoxDecoration(color: const Color(0xFFEFEAFF), borderRadius: BorderRadius.circular(14)), child: const Icon(Icons.directions_run_rounded, color: _purple)), const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Squat', style: TextStyle(fontWeight: FontWeight.w900, color: _ink)), const SizedBox(height: 4), Text('${session.reps} reps • ${session.completedAt.day}/${session.completedAt.month}/${session.completedAt.year}', style: TextStyle(color: _muted, fontSize: 12))])), Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('${session.averageScore}', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: _ink)), const Text('score', style: TextStyle(fontSize: 10, color: _muted))]) ])); }
